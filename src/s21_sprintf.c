...
int s21_sprintf(char *str, const char *format, ...) {
  va_list va;
  va_start(va, format); // Инициализация списка аргументов.
  int error = 1; // Флаг ошибки, по умолчанию ошибок нет.
  int processed = 0; // Счётчик обработанных символов.
  struct PreparedFormatData prepared_format_data = {0}; // Структура для хранения информации о формате.
  str[0] = '\0'; // Обнуляем строку.
  while (*format) { // Проходим по всей строке формата.
    if (error) {
      s21_memset(&prepared_format_data, 0, sizeof(prepared_format_data)); // Обнуляем структуру информации о формате.
      if (*format == '%') { // Если встречаем символ '%', начинаем обработку спецификатора.
        format++;
        format = sprintf_parser(format, &prepared_format_data, &error, va); // Парсим спецификатор формата.
        specifier_handler(&str, &prepared_format_data, va, processed); // Обрабатываем спецификатор и записываем в строку.
        processed += prepared_format_data.counter; // Увеличиваем счётчик обработанных символов.
      } else {
        write_to_str(str, format); // Записываем обычный символ в строку.
        processed++;
        format++;
      }
    } else {
      printf("error"); // Если произошла ошибка, выводим сообщение об ошибке.
      *str = '\0'; // Обнуляем строку.
      break;
    }
  }
  va_end(va); // Завершаем работу со списком аргументов.
  return processed; // Возвращаем количество обработанных символов.
}
...
const char *sprintf_parser(const char *format,
  ...
  format = get_flags(format, prepared_format_data); // Обрабатываем флаги формата (например, '-', '+', ' ').
  format = get_width(format, prepared_format_data, va); // Обрабатываем ширину поля (например, число перед спецификатором).
  format = get_precision(format, prepared_format_data, va); // Обрабатываем точность (например, число после точки).
  format = get_length(format, prepared_format_data); // Обрабатываем длину (например, 'l', 'h').
  format = get_specifier(format, prepared_format_data, error); // Обрабатываем спецификатор (например, 'd', 's').
...
int x_to_upper(char *value_str) {
  for (size_t i = 0; i < s21_strlen(value_str); i++) {
